...
\item You fixed all affected documentation.
...
\side{
  ...
  \textbf{Estimated Effort:} \\[1ex]
  \geticsval{/H2} \\[4ex] % Flo: maybe more effort (6-7 h), esp. for getting started with Elektra-Development and the behaviour of the CI-pipeline (checks, formatting, testing, ...). markus: reduced to 3 reviews.
  \textbf{Points:} \\[1ex]
  10 \\[4ex]
  \textcolor{aktiv}{
  \textbf{Tasks:} \\[1ex]
...
\item Improve the readability of some sentence you found hard to understand.
...
Create at least two PRs in which your team:
...
Furthermore, create at least two reviews of T1 PRs.
\\\review{}
...
Furthermore, create at least three reviews of T2 PRs.
\\For the reviews, make sure that:
\begin{itemize}
...
\side{
  \textbf{Deadline:} \\[1ex]
  \geticsval{/P1 Midterm Report\timekey} \\[4ex]
  \textbf{Reviews until:} \\[1ex]
...
\side{
  ...
  \geticsval{/M11 Presentations\timekey} \\[4ex]
  \textbf{Estimated Effort:} \\[1ex]
  \geticsval{/P2}h \\[4ex]
  \textbf{Points:} \\[1ex]
  0 \\[4ex]
...
\item The presentation can be on any week but must be at latest on \geticsval{/M11 Presentations}.
