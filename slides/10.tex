...
\begin{frame}
	...
	\begin{itemize}[<+-| alert@+>]
	\item began at CFengine Workshop at LISA 2001
...
\begin{frame}
	...
	What can we learn from system administration?
...
\begin{frame}
	...
	\item to also support manual workflows and scripts
...
\begin{frame}
	...
	Decide the \textbf{changeability} per key:

	\begin{itemize}[<+-| alert@+>]
	\item Who is responsible (end user, packages, admin manually or by CM).
	\item In which namespaces the key is searched for (cascading lookup).
	...
	Ownership of every key must be very clear.
	\end{alertblock}
\end{frame}
...
\begin{frame}
	...
	%TODO: split different views of abstractions?
	\begin{itemize}[<+-| alert@+>]
	\item Bits in configuration files
	\item Key/value view of configuration settings
	\item Specification of the view
	\item Arbitrary number of abstractions with CM code
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}[<+-| alert@+>]
	\item factor processes into containers to avoid setting overlaps.
...
\begin{frame}
	...
	\item use all help you can get: e.g.\ build tools, virtualization, package managers, distributions
	\item complexity into CM vs.\ abstractions in CM vs.\ application's configuration specification
	\item create stateless transactions (get/set)
	\end{itemize}
\end{frame}
